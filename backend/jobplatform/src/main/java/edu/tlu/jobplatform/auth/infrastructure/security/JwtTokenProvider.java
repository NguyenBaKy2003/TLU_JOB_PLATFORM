...
public class JwtTokenProvider {
    ...
    public boolean validateToken(String token) {
        ...
        catch (MalformedJwtException e)     { log.warn("JWT malformed"); }
        catch (SecurityException e)         { log.warn("JWT signature invalid"); }
        catch (IllegalArgumentException e)  { log.warn("JWT empty/null"); }
        return false;
    }

    // ── Đọc claims ────────────────────────────────────────────────

    public UUID   extractUserId(String token)  { return UUID.fromString(parseClaims(token).getSubject()); }
    public String extractEmail(String token)   { return parseClaims(token).get("email",  String.class); }
    public String extractRole(String token)    { return parseClaims(token).get("role",   String.class); }
    public String extractTokenId(String token) { return parseClaims(token).get("tid",    String.class); }
    public String extractJti(String token)     { return parseClaims(token).getId(); }
    ...
    public long getAccessTokenExpirySeconds() { return accessTokenExpiryMs / 1000; }
