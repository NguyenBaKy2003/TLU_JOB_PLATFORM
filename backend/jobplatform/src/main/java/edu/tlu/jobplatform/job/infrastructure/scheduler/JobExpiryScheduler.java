...
public class JobExpiryScheduler {
    ...
    public void expireOverdueJobs() {
        ...
        List<JobPost> overdueJobs = jobPostRepository
                .findByStatusAndDeadlineBefore(JobStatus.PUBLISHED, today);

        if (overdueJobs.isEmpty()) {
            log.debug("No overdue jobs found on {}", today);
            return;
        }
        ...
        for (JobPost job : overdueJobs) {
            try {
                ...
                log.error("Failed to expire job id={}: {}", job.getId(), e.getMessage(), e);
            }
        }
