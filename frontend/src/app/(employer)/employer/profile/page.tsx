...
import { TeamMembersSection } from "@/presentation/components/employer/company/TeamMembersSection";
import { GallerySection } from "@/presentation/components/employer/company/GallerySection";
import { DocumentsSection } from "@/presentation/components/employer/company/DocumentsSection";
  ...
  if (error || !profile) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <p className="text-[16px] text-red-500">{error ?? "Không thể tải hồ sơ công ty"}</p>
        <button onClick={loadProfile}
          className="px-4 py-2 text-[16px] font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700">
          Thử lại
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 items-start">
      ...
      <div className="w-full lg:flex-1 lg:min-w-0 flex flex-col gap-4">
        ...
        {/* Documents Section - chỉ hiển thị nếu có hoặc đang chờ xác thực */}
        {(profile.documents || profile.verificationStatus !== "VERIFIED") && (
          <DocumentsSection
