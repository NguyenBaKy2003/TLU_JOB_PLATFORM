...
export default function CreateStreamPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toastError("Tạo phiên thất bại", extractErrorMessage(e));
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex gap-3 pt-2">
            {step < 3 ? (
              <button
                type="button"
                onClick={handleNext}
                className="flex-1 bg-slate-800 text-white text-[16px] font-semibold py-3 rounded-xl
                  hover:bg-slate-900 transition-colors flex items-center justify-center gap-2"
              >
                Tiếp theo
              ...
              >
                {loading ? (
                  ...
                  <><Radio className="w-4 h-4" /> Tạo phiên stream</>
                )}
              </button>
            )}
          </div>
