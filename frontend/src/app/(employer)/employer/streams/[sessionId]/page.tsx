...
import {
  Radio, Play, ExternalLink, ArrowLeft,
  Calendar, Users, Clock, Briefcase, Loader,
} from "lucide-react";
...
export default function SessionDetailPage() {
  ...
  const [analytics, setAnalytics] = useState<StreamAnalytics | null>(null); // ← THÊM
  ...
  useEffect(() => {
    service.getSession(sessionId)
      .then(async s => {
        setSession(s);
        // Fetch analytics chỉ khi session đã ENDED
       if (s.status === "ENDED") {
            setAnalyticsLoading(true);
            try {
              const a = await service.getAnalytics(sessionId);
              setAnalytics(a);
            } catch {
              // bỏ qua
            } finally {
  ...
  const handleInvite = async (sId: string, candidateId: string, slotId: string) =>
    service.inviteToSlot(sId, { candidateId, slotId });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        {/* ── Analytics — chỉ hiển thị khi ENDED ── */}
        {isEnded && (
          <AnalyticsCard
            peakViewerCount={analytics?.peakViewerCount}
            totalViewerCount={analytics?.totalViewerCount}
            totalWatchSeconds={analytics?.totalWatchSeconds}
            qaQuestionCount={analytics?.qaQuestionCount}
            pollResponseCount={analytics?.pollResponseCount}
            applyClickCount={analytics?.applyClickCount}
            loading={analyticsLoading}                  
            onClick={() => router.push(`/employer/streams/${sessionId}/analytics`)}
          />
        )}
      </div>
    </div>
