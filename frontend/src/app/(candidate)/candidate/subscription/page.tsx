...
function yearlyDiscount(plan: CandidateSubscriptionPlan): number {
  ...
  return monthly12 > 0
    ? Math.round(((monthly12 - plan.priceYearly) / monthly12) * 100)
    : 0;
}
...
// ── PaymentModal adapter ──────────────────────────────────────────────────────
// PaymentModal expects SubscriptionPlan (Company type) — we create a thin adapter
// so we don't duplicate the modal component.
...
function toSubscriptionPlan(p: CandidateSubscriptionPlan): SubscriptionPlan {
  return {
    ...
    // Company-specific fields not used by modal UI — set safe defaults
    jobPostLimit:      0,
    featuredJobLimit:  0,
    cvViewLimit:       0,
    aiFeatures:        p.aiCvWriter,
    analyticsAccess:   p.profileAnalytics,
...
export default function CandidateSubscriptionPage() {
  ...
  const handleConfirmPayment = useCallback(async () => {
    ...
    try {
      const result = await service.purchase({ planId: selectedId, yearly });
      // Redirect to VNPay
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-2xl">
            <p className="text-xs text-gray-500 text-center">
              ...
              Xem <a href="/pricing#compare" className="text-violet-600 hover:underline">so sánh đầy đủ</a>.
            </p>
          </div>
        </div>
