...
import {
  ...
  Mic, Cpu, TrendingUp, Home, Loader2, Package,
...
function QuotaBar({ label, used, limit, icon }: { label: string; used: number; limit: number; icon: React.ReactNode }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-8 h-8 rounded-lg bg-violet-50 flex items-center justify-center text-violet-500 shrink-0">{icon}</div>
...
function ReceiptRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between py-3.5">
      ...
      <span className={`text-[16px] font-semibold ${highlight ? "text-violet-600" : "text-gray-800"}`}>{value}</span>
    </div>
  );
}
...
export default function CandidatePaymentSuccessPage() {
  ...
  if (error || !sub) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-emerald-50 flex items-center justify-center px-4">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-emerald-50 px-4 py-12">
      <div className="pointer-events-none fixed inset-0 overflow-hidden -z-10">
        ...
        <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-emerald-100 opacity-40 blur-3xl" />
      </div>
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 px-8 py-10 text-center">
          ...
          <p className="text-[16px] text-gray-500 max-w-xs mx-auto leading-relaxed">
            Gói <span className="font-semibold text-gray-800">{sub.planCode}</span> đã được kích hoạt. Bắt đầu nâng cao hồ sơ của bạn ngay hôm nay.
          </p>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-violet-600 to-violet-500 px-6 py-4 flex items-center justify-between">
            <div>
              <p className="text-violet-100 text-xs font-medium uppercase tracking-widest">Hóa đơn</p>
              <p className="text-white font-bold text-lg mt-0.5">#{sub.currentPaymentId?.slice(-8).toUpperCase() ?? "—"}</p>
            </div>
            <div className="text-right">
              <p className="text-violet-200 text-xs">Trạng thái</p>
        ...
        {quota && (
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              <QuotaBar
                label="Ứng tuyển"
                used={quota.applicationQuota?.used}
                limit={quota.applicationQuota?.limit}
                icon={<FileText size={14} />}
              />
              <QuotaBar
                label="CV Boost"
                used={quota.cvBoostQuota?.used}
                limit={quota.cvBoostQuota?.limit}
                icon={<TrendingUp size={14} />}
              />
              <QuotaBar
                label="Thông báo việc làm"
                used={quota.jobAlertQuota?.used}
                limit={quota.jobAlertQuota?.limit}
                icon={<FileText size={14} />}
              />
              <QuotaBar
                label="Phỏng vấn thử"
                used={quota.mockInterviewQuota?.used}
                limit={quota.mockInterviewQuota?.limit}
                icon={<Mic size={14} />}
              />
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            <FeatureBadge active={sub.aiCvWriter} label="AI viết CV" icon={<Cpu size={13} />} />
            <FeatureBadge active={sub.advancedFilters} label="Bộ lọc nâng cao" icon={<TrendingUp size={13} />} />
            <FeatureBadge active={sub.profileAnalytics} label="Phân tích hồ sơ" icon={<TrendingUp size={13} />} />
            <FeatureBadge active={sub.salaryInsights} label="Thông tin lương" icon={<TrendingUp size={13} />} />
          </div>
        </div>
        ...
        <p className="text-center text-xs text-gray-400 pb-4">
          ...
          <a href="mailto:support@jobplatform.vn" className="text-violet-500 hover:underline">support@jobplatform.vn</a>{" "}
          nếu cần hỗ trợ.
        </p>
      </div>
    </div>
