...
// Copy this file into each folder (Next.js colocation) OR move to a shared components dir.
...
function resolveReason(raw: string | null): { title: string; desc: string } {
  ...
  return REASON_MAP[raw] ?? {
    ...
    desc: decodeURIComponent(raw).length < 100 ? decodeURIComponent(raw) : "Đã có lỗi xảy ra trong quá trình xử lý giao dịch.",
  };
}
