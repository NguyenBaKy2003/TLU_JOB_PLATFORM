...
function PlanRow({ plan, onEdit, onToggle, formatPrice, toggling }: PlanRowProps) {
  return (
    ...
    <tr className="border-b border-gray-50 hover:bg-gray-50/60 transition-colors">
      ...
      <td className="px-5 py-3.5">
        <span className={`px-2.5 py-1 text-[11px] font-semibold rounded-full border
          ${SUB_STATUS_STYLES[sub.status] ?? "bg-gray-100 text-gray-600"}`}>
          {sub.status}
        </span>
      </td>
...
export default function AdminSubscriptionPage() {
  ...
  const loadPlans = useCallback(async () => {
    setLoading(true);
    try {
      // Repository normalize prices — không cần coerce thêm ở đây
  ...
  const handleSavePlan = useCallback(async (payload: PlanPayload) => {
    try {
      if (editingPlan === "new") {
        ...
        // editingPlan.id là string — không cần ép kiểu
  ...
  const handleToggle = useCallback(async (plan: SubscriptionPlan) => {
    // plan.id là string — không cần ép kiểu
    ...
    <div className="flex flex-col gap-6">
      ...
      {tab === "plans" && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <p className="text-xs text-gray-500">
              <strong className="text-gray-800">{plans.length}</strong> gói dịch vụ
            </p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden animate-pulse">
              {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="flex gap-4 px-5 py-4 border-b border-gray-50">
                  {[100, 80, 60, 60, 60, 60, 40, 40].map((w, j) => (
                    ...
                    <tr className="border-b border-gray-50">
                      {["Gói","Giá","Thời hạn","Tin đăng","Tin nổi bật","Xem CV","AI","Analytics",""].map(h => (
                        <th key={h} className="px-5 py-3.5 text-left text-[11px] font-semibold
                          text-gray-400 uppercase tracking-wide whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
      ...
      {tab === "subscriptions" && (
        <div className="flex flex-col gap-4">
          ...
          {/* Banner: API chưa tồn tại ở backend */}
          <div className="flex items-start gap-3 px-4 py-3 bg-amber-50 border
            ...
            <p>
              Chức năng này chưa có API từ backend. Cần tạo endpoint{" "}
              <code className="font-mono text-xs bg-amber-100 px-1 py-0.5 rounded">
                GET /api/v1/admin/subscriptions
              </code>{" "}
              để hiển thị danh sách subscription của tất cả công ty.
            </p>
          </div>
                ...
                <table className="w-full min-w-[700px]">
                  ...
                  <tbody>
                    {subs.length === 0 ? (
                      ...
                      subs.map(sub => (
                        <SubRow key={sub.id} sub={sub} formatPrice={n => service.formatPrice(n)} />
                      ))
                    )}
                  </tbody>
                </table>
