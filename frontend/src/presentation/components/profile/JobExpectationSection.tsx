            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={draft.minSalary}
                  ...
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-gray-300" />
              </div>
              <div>
            ...
            {([...draft.contractTypes, ...draft.levels]).length > 0 && (
              <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100">
                {draft.contractTypes.map((t) => (
                  <span key={t} className="flex items-center gap-1.5 px-3 py-1 text-sm bg-blue-50 text-blue-700 rounded-full">
                    {LABEL(CONTRACT_OPTIONS, t)}
                    <button onClick={() => toggle("contractTypes", t)}><X size={12} /></button>
                  </span>
                ))}
                {draft.levels.map((l) => (
                  <span key={l} className="flex items-center gap-1.5 px-3 py-1 text-sm bg-blue-50 text-blue-700 rounded-full">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {[...(desired?.contractTypes ?? []), ...(desired?.levels ?? [])].map((tag) => (
              <span key={tag} className="px-3 py-1.5 text-sm text-gray-700 bg-gray-100 rounded-full border border-gray-200">
