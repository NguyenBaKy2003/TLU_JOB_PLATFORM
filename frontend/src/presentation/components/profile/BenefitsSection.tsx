      ...
      {editing ? (
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {PRESET_BENEFITS.map((b) => (
              <label key={b} className="flex items-center gap-2 cursor-pointer p-2 rounded-lg hover:bg-gray-50"
                ...
                <span className="text-[16px] text-gray-700">{b}</span>
              </label>
            ))}
          </div>
          ...
          {draft.filter((b) => !PRESET_BENEFITS.includes(b)).length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4 pt-2 border-t border-gray-100">
              {draft.filter((b) => !PRESET_BENEFITS.includes(b)).map((b) => (
                <span key={b} className="flex items-center gap-1.5 px-3 py-1 text-[16px] bg-gray-100 text-gray-700 rounded-full">
        ...
        <div className="flex flex-wrap gap-2">
          {currentNames.length > 0 ? currentNames.map((b) => (
            <span key={b} className="px-3 py-1.5 text-[16px] text-gray-700 bg-gray-100 rounded-full border border-gray-200">{b}</span>
          )) : <p className="text-[16px] text-gray-400 italic">Chưa có phúc lợi</p>}
        </div>
      )}
    </SectionWrapper>
  );
