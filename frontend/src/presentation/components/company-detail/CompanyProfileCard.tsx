...
import {
  Globe, MapPin, Users, Mail, Phone,
  Bookmark, Share2, Bell, Building2, Calendar, CheckCircle,
} from "lucide-react";
...
export function CompanyProfileCard({ company }: Props) {
  ...
  const hasPlan    = company.planCode && company.planCode !== "FREE_COMPANY";
  ...
  return (
    <div className={`relative bg-white rounded-2xl shadow-sm overflow-hidden
      ${hasPlan
        ? "border-[1.5px] border-violet-300"
      ...
      {/* Plan tint */}
      {hasPlan && (
        <div className="absolute inset-0 bg-violet-50/15 pointer-events-none" />
      )}
      ...
      <div className="relative p-6">
        ...
        {hasPlan && (
          <div className="mb-4">
            <PlanBadge planCode={company.planCode as CompanyPlanCode} size="md" />
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center">
          ...
          {/* Logo */}
          <div className={`w-20 h-20 rounded-2xl border-2 overflow-hidden shrink-0 bg-white
            flex items-center justify-center shadow-sm
            ${hasPlan ? "border-violet-200 ring-2 ring-violet-100 ring-offset-1" : "border-gray-100"}`}>
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h1 className={`text-xl font-bold
                    ${hasPlan ? "text-violet-900" : "text-gray-900"}`}>
                    {company.name}
                  </h1>
                  {isVerified && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5
                      bg-green-50 text-green-700 text-[10px] font-medium rounded-full">
