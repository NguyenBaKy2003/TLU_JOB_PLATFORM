...
function JobCard({ job, companyName }: { job: CompanyDetail["jobs"][0]; companyName: string }) {
  return (
    <Link href={`/jobs/${job.id}`}
      className="flex items-start gap-4 p-4 bg-white border border-gray-100 rounded-2xl
          ...
          {job.tags.map(tag => (
            <span key={tag}
              className={`px-2 py-0.5 text-[11px] font-medium rounded-full border ${
                TAG_STYLES[tag] ?? "bg-gray-50 text-gray-600 border-gray-200"
              }`}>
              {tag}
            </span>
          ))}
...
export function JobsTab({ company }: { company: CompanyDetail }) {
  return (
    <div>
