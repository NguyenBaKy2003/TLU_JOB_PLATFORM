// D:\TLU_JOB_PLATFORM\frontend\src\presentation\components\admin\subscription\CandidatePlanFormModal.tsx
...
const DEFAULT: CandidatePlanPayload = {
  ...
  applicationLimit: 5, cvBoostLimit: 0,
  jobAlertLimit: 3, mockInterviewLimit: 0,
  aiCvWriter: false, salaryInsights: false,
  profileAnalytics: false, advancedFilters: false,
  durationDays: 30, free: false,
};
...
export function CandidatePlanFormModal({ plan, onSave, onCancel }: Props) {
  ...
  const [form, setForm] = useState<CandidatePlanPayload>(
    plan
      ? {
          ...
          jobAlertLimit:    plan.jobAlertLimit,
          mockInterviewLimit: plan.mockInterviewLimit,
          aiCvWriter:       plan.aiCvWriter,
          salaryInsights:   plan.salaryInsights,
          profileAnalytics: plan.profileAnalytics,
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof CandidatePlanPayload, string>>>({});

  const set = <K extends keyof CandidatePlanPayload>(key: K, val: CandidatePlanPayload[K]) => {
    setForm(p => ({ ...p, [key]: val }));
...
const validate = (): boolean => {
  ...
  if (!form.free && form.durationDays < 1)
    e.durationDays = "Phải ≥ 1 ngày";

  if (form.free && (form.priceMonthly > 0 || form.priceYearly > 0))
    e.priceMonthly = "Gói miễn phí phải có giá = 0";
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden mb-8">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="text-[16px] font-semibold text-gray-800">
            {isEdit ? `Chỉnh sửa gói Candidate: ${plan?.name}` : "Tạo gói Candidate mới"}
          </h3>
        ...
        <div className="px-6 py-5 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Tên gói" required>
                <input value={form.name} onChange={e => set("name", e.target.value)}
                  placeholder="Gói Chuyên Nghiệp" className={inputCls} />
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Field label="Giá tháng" hint={form.free ? "Tự động = 0" : "0 = Miễn phí"}>
                ...
                {errors.priceMonthly && <p className="text-[11px] text-red-500">{errors.priceMonthly}</p>}
              </Field>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <LimitInput label="Lượt ứng tuyển/tháng"
                value={form.applicationLimit}
                onChange={v => set("applicationLimit", v)} />
              <LimitInput label="Lượt đẩy CV"
                value={form.cvBoostLimit}
                onChange={v => set("cvBoostLimit", v)} />
              <LimitInput label="Job Alert"
                value={form.jobAlertLimit}
                onChange={v => set("jobAlertLimit", v)} />
              <LimitInput label="Buổi phỏng vấn thử"
                value={form.mockInterviewLimit}
                onChange={v => set("mockInterviewLimit", v)} />
            </div>
          </section>
          ...
          <section className="flex flex-col gap-4 p-4 bg-gray-50 rounded-2xl">
            ...
            <ToggleRow
              label="AI CV Writer"
              desc="Tạo và tối ưu CV bằng AI, gợi ý từ khóa phù hợp"
              checked={form.aiCvWriter}
              onChange={v => set("aiCvWriter", v)}
            />
            <ToggleRow
              label="Phân tích mức lương (Salary Insights)"
              desc="Xem thống kê mức lương theo vị trí, ngành nghề, khu vực"
              checked={form.salaryInsights}
              onChange={v => set("salaryInsights", v)}
            />
            <ToggleRow
              label="Phân tích hồ sơ (Profile Analytics)"
              desc="Thống kê lượt xem hồ sơ, tỉ lệ phản hồi từ nhà tuyển dụng"
              checked={form.profileAnalytics}
              onChange={v => set("profileAnalytics", v)}
            />
            <ToggleRow
              label="Bộ lọc nâng cao (Advanced Filters)"
              desc="Lọc việc làm theo mức lương, phúc lợi, đánh giá công ty..."
              checked={form.advancedFilters}
              onChange={v => set("advancedFilters", v)}
            />
          </section>
