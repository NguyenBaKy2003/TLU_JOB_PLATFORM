...
export function CompaniesAISearchBar({ keyword, location, onSearch }: CompaniesAISearchBarProps) {
  ...
  // Sync when parent state changes (e.g. URL navigation or handleClearAll)
  ...
  const { data: autocompleteData, isLoading: isAutocompleteLoading } = useQuery({
    queryKey: ["ai", "autocomplete", "companies", debouncedKw],
  ...
  const handleSearch = useCallback((kw: string, loc?: string) => {
    ...
    setIsOpen(false);
    onSearch(kw.trim(), loc ?? draftLocation);
  }, [draftLocation, onSearch, trackSearch]);

  const handleSelectSuggestion = (s: AutocompleteSuggestion) => {
    setDraftKeyword(s.query);
    handleSearch(s.query, draftLocation);
  };
  ...
  return (
    <div ref={wrapperRef} className="relative max-w-3xl mx-auto">
      <div className="flex flex-col sm:flex-row rounded-2xl bg-white shadow-2xl overflow-visible">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-6 py-3.5
            ...
            hover:bg-blue-700 transition-colors rounded-b-2xl sm:rounded-b-none sm:rounded-r-2xl"
        >
