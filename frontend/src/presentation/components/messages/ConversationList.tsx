...
interface Props {
  ...
  onRetry?:      () => void;
  /** ID của conversation vừa nhận tin mới (để flash) */
  flashId?:      string | null;
}

export function ConversationList({
  conversations, loading, error, activeId, onSelect, onRetry, flashId,
}: Props) {
  const [query, setQuery] = useState("");

  // Reset flash sau 1.5s
  ...
  useEffect(() => {
    ...
    flashTimer.current = setTimeout(() => setLocalFlashId(null), 1500);
    return () => { if (flashTimer.current) clearTimeout(flashTimer.current) };
  }, [flashId]);

  const filtered = query.trim()
    ? conversations.filter(c =>
        c.otherParticipantName.toLowerCase().includes(query.toLowerCase()) ||
      ...
      <div className="px-4 pt-4 pb-3 border-b border-gray-50 shrink-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            {totalUnread > 0 && (
              <span className="px-2 py-0.5 text-[11px] font-bold text-blue-600
