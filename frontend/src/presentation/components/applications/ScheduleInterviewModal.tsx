...
import type { ScheduleInterviewRequest } from "@/domain/models/Application";
...
interface Props {
  ...
  onConfirm:     (req: ScheduleInterviewRequest) => Promise<void>;
  onCancel:      () => void;
}

export function ScheduleInterviewModal({ candidateName, onConfirm, onCancel }: Props) {
  ...
  const handleConfirm = async () => {
    if (!validate()) return;
    setSaving(true);
    try { await onConfirm({ scheduledAt, location, note: note || undefined }); }
    catch {}
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <div className="px-4 py-3 bg-blue-50 rounded-xl">
            <p className="text-xs text-blue-600 font-medium">Ứng viên: <strong>{candidateName}</strong></p>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="datetime-local" value={scheduledAt} min={minDate}
            ...
            {errors.scheduledAt && <p className="text-[11px] text-red-500">{errors.scheduledAt}</p>}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input value={location} onChange={e => { setLocation(e.target.value); setErrors(p => ({ ...p, location: undefined })); }}
