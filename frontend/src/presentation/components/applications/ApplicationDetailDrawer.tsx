...
import { X, FileText, Clock,
         MapPin, User, Mail, Phone }    from "lucide-react";
...
// Statuses the employer can move to (simplified; extend as needed)
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-5">
          ...
          {!loading && detail && (
            <>
              ...
              {detail.candidate && (
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl">
                  {detail.candidate.avatarUrl
                    ? <img src={detail.candidate.avatarUrl} alt={detail.candidate.fullName}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900">{detail.candidate.fullName}</p>
                  ...
                  <div className="relative">
                    ...
                    {showStatusMenu && (
                      <div className="absolute right-0 top-full mt-1 z-10 bg-white border border-gray-200
                        rounded-xl shadow-lg min-w-[180px] py-1 flex flex-col">
                        {allowedNext.map(s => (
                          <button key={s} onClick={() => handleStatusChange(s)}
                            className="px-4 py-2 text-xs text-left text-gray-700 hover:bg-gray-50 transition-colors">
              ...
              {/* Status note input (employer only) */}
              {role === "employer" && allowedNext.length > 0 && (
                <div className="flex flex-col gap-1">
                  <label className="text-xs text-gray-400">Ghi chú khi đổi trạng thái (tùy chọn)</label>
                  <input
                    value={statusNote}
              ...
              <div className="grid grid-cols-2 gap-2">
                <InfoItem label="Ngày nộp" value={new Date(detail.appliedAt).toLocaleDateString("vi-VN")} />
