import {
  CalendarDays, FileText, TrendingUp, Mic,
  Cpu, SlidersHorizontal, BarChart2, DollarSign, Bell,
} from "lucide-react";
import type { CandidateSubscription, CandidateQuotaResult, CandidateQuota } from "@/domain/models/CandidateSubscription";
...
function QuotaBar({
  ...
  const color       = pct >= 90 ? "bg-red-500" : pct >= 70 ? "bg-yellow-400" : "bg-violet-500";
  ...
  return (
    <div>
      ...
      {isUnlimited ? (
        <p className="text-[11px] text-emerald-600 font-medium">Không giới hạn</p>
      ) : (
...
function FeatureChip({ active, label, icon }: { active: boolean; label: string; icon: React.ReactNode }) {
  return (
...
export function CandidateCurrentSubscriptionCard({ sub, quota }: Props) {
  ...
  const mockInterviewQuota = quota?.mockInterviewQuota ?? sub.mockInterviewQuota ?? null;
  const jobAlertQuota      = quota?.jobAlertQuota      ?? sub.jobAlertQuota      ?? null;
  ...
  const statusStyle = statusMap[sub.status] ?? { label: sub.status, className: "bg-gray-100 text-gray-500 border-gray-200" };
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">{sub.yearly ? "Chu kỳ hàng năm" : "Chu kỳ hàng tháng"}</p>
        </div>
        <div className={`px-3 py-1 text-xs font-bold rounded-full border shrink-0 ${statusStyle.className}`}>
          {statusStyle.label}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <QuotaBar label="Ứng tuyển"       icon={<FileText size={12} />}    quota={applicationQuota} />
        <QuotaBar label="CV Boost"         icon={<TrendingUp size={12} />}  quota={cvBoostQuota} />
        <QuotaBar label="Phỏng vấn thử"   icon={<Mic size={12} />}         quota={mockInterviewQuota} />
        <QuotaBar label="Thông báo việc"  icon={<Bell size={12} />}         quota={jobAlertQuota} />
      </div>

      {/* Feature chips */}
      <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 mt-5 pt-4 border-t border-gray-50">
        <FeatureChip active={sub.aiCvWriter}      label="AI viết CV"      icon={<Cpu size={11} />} />
        <FeatureChip active={sub.advancedFilters}  label="Bộ lọc nâng cao" icon={<SlidersHorizontal size={11} />} />
        <FeatureChip active={sub.profileAnalytics} label="Phân tích hồ sơ" icon={<BarChart2 size={11} />} />
        <FeatureChip active={sub.salaryInsights}   label="Thông tin lương" icon={<DollarSign size={11} />} />
      </div>
    </div>
  );
}