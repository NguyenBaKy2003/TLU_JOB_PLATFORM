      ...
      <div className="bg-slate-50 rounded-xl border border-slate-200 divide-y divide-slate-200">
        {infoItems.map(({ label, value }) => (
          <div key={label} className="flex gap-4 px-4 py-3.5">
            ...
            <span className="text-[16px] text-slate-700 font-medium">{value}</span>
          </div>
        ))}
      </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {data.interviewSlots.map((slot, i) => (
              <div
                key={i}
                className="flex items-center gap-3 text-[16px] text-slate-600 bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl"
              >
