...
export function CompanyHero({
  ...
  // Preview giống ProfileHero
  const [logoPreview, setLogoPreview] = useState<string | undefined>(
    profile.logoUrl ?? undefined
  );
  const [coverPreview, setCoverPreview] = useState<string | undefined>(
    profile.coverImageUrl ?? undefined
  );

  // Handle upload giống ProfileHero
  ...
  const initials =
    (profile.name ?? "").slice(0, 2).toUpperCase() || "CO";
        ...
        {coverPreview && (
          <Image
            ...
            className={`object-cover transition-opacity ${
              coverUploading ? "opacity-60" : "opacity-100"
            }`}
          />
        )}
      ...
      <div className="px-6 pb-6 -mt-10 relative">
        <div className="flex flex-col sm:flex-row sm:items-end gap-4">
          ...
          <div
            ...
            onClick={() =>
              !logoUploading && logoRef.current?.click()
            }
          >
            <div
              className="w-20 h-20 rounded-2xl border-4 border-white bg-white shadow-md overflow-hidden
            ...
            >
              {logoPreview ? (
                <Image
                  ...
                  alt={profile.name}
                  fill
                  className={`object-cover rounded-2xl transition-opacity ${
                    logoUploading ? "opacity-60" : "opacity-100"
                  }`}
                />
              ) : (
            ...
            {onLogoChange && (
              <div
                className="absolute -bottom-1 -right-1 w-6 h-6 bg-white border border-gray-200
